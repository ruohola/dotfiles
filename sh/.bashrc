...
alias vtc='vim ~/.tmux.conf && tmux source-file ~/.tmux.conf 2> /dev/null || true'
...
mux () {
    ...
    local out key session selected current_session
...
alias glb='git log --compact-summary "$(__git_default_remote_branch)"..'  # Mnemonic: git log branch
alias glbb='glp "$(__git_default_remote_branch)"..'
alias glbr='glbb --reverse'
...
export FZF_IGNORES='Applications,Library,Movies,Music,Pictures,node_modules,venv,.DS_Store,.Trash,.cache,.gradle,.git,.m2,.mypy_cache,.next,.npm,.pyenv,.pytest_cache,.stack,.temp,__pycache__,build,dist'
